...
async def temporary_chat_completions(request: OpenAIChatRequest, http_request: Request):
    # --- LOCAL PATCH (not upstream) ---
    # Optional per-request Gemini cookie override, used by callers that manage
    # their own pool of Gemini accounts (e.g. a multi-tenant gateway sitting in
    # front of this service) instead of relying on this process's single
    # globally-configured account. Only wired into this endpoint because it's
    # the one Gemini WebAPI chat path with no SessionRegistry/conversation
    # persistence to fight with — see temporary_chat.py for the actual client
    # construction. Not part of the upstream API contract; ignored entirely
    # unless both headers are present.
    psid = http_request.headers.get("X-Gemini-1PSID")
    psidts = http_request.headers.get("X-Gemini-1PSIDTS")
    if psid and psidts:
        object.__setattr__(request, "_gemini_cookie_override", {"psid": psid, "psidts": psidts})
    # --- END LOCAL PATCH ---
