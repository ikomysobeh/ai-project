...
4. **Rotation loop** — for each candidate account in order:
   ...
   - **Success** → mark the account `active` + `last_used_at = now()`, log a `usage_record` (tokens pulled from the response's `usage` object), return the response verbatim.
...
1. Validate the file (`app/Http/Requests/Api/StoreDocumentRequest.php`) — max 5120 KB, extension must be `.txt` or `.md` (checked via a closure, not Laravel's `mimes` rule, because Symfony's MIME guesser doesn't reliably recognize `.md`).
...
2. Reads the file text from storage.
...
5. On success, `Document.status = 'ready'`; on any exception, logs it and sets `Document.status = 'failed'`.
